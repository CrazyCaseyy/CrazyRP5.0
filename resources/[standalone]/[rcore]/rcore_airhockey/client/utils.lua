...
function ChooseAfter(tbl, value)
    local found = false
    if #tbl > 0 then
        ...
        end) -- sorted order for consistency
...
function ChooseBefore(tbl, value)
...
function FullscreenPrompt(caption, message, onclose, bottomText)
    Citizen.CreateThread(function()
        ...
        ShowHelpButtons({{
            key = 176,
            title = Translation.Get("UI_CONFIRM")
        }})
