...
var maxScore = 3;
const DispatchTable = {
    SavesChanged: function (pid, saves) {
        SavesChanged(pid, saves);
    },
    PlayPuckSound: function () {
        PlayPuckSound();
    },
    PlayPaddleSound: function () {
        PlayPaddleSound();
    },
    ShowCounter: function (counterText, color) {
        ShowCounter(counterText, color);
    },
    ToggleLoading: function (loadingText) {
        ToggleLoading(loadingText);
    },
    ResetSession: function () {
        ResetSession();
    },
    ToggleHockeyUI: function (visible) {
        ToggleHockeyUI(visible);
    },
    ToggleGameInfo: function (visible) {
        ToggleGameInfo(visible);
    },
    ToggleBettingBar: function (visible, text) {
        ToggleBettingBar(visible, text);
    },
    PlayGoalSound: function () {
        PlayGoalSound();
    },
    ShowAnnounce: function (announceText) {
...
function RefreshPlayerLayout() {
    for (let i = 1; i <= 2; i++) {
        ...
        document.getElementById("p" + i + "saves").textContent = safeSaves;
...
function SetPing(ping) {
    var pingRow = document.getElementById("pingRow");
    if (typeof ping === "string") {
        pingRow.innerHTML = ping;
        return;
    }
    pingRow.innerHTML = ping + " ms";
}
...
function PlayPaddleSound() {
    ...
    var audio = new Audio();
    audio.src = "sounds/paddle" + rnd + ".wav";
    audio.volume = 0.1;
    audio.play();
}
