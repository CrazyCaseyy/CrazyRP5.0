local leaveRateLimit = {}

RegisterCommand('bowlingleave', function(source)
    if leaveRateLimit[source] then
        print("Player tried to leave bowling while ratelimited", source)
        return
    end

    leaveRateLimit[source] = true

    Citizen.SetTimeout(1000, function()
        leaveRateLimit[source] = false
    end)
