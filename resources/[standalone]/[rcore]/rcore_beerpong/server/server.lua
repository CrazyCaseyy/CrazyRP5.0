...
RegisterNetEvent(TriggerName("VisibleScoreStatusForPlayer"), function(identifier, status)
    local source = source
    local playerInitialPosition = GetEntityCoords(GetPlayerPed(source))

    for k, v in pairs(GetPlayers()) do
        k = type(v) == "string" and tonumber(v) or v
        local targetPed = GetPlayerPed(k)
        if #(playerInitialPosition - GetEntityCoords(targetPed)) < 30 then
            TriggerClientEvent(TriggerName("VisibleScoreStatusForPlayer"), k, identifier, status, source)
        end
    end
end)
